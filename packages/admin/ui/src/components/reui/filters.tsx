...
import { AlertCircleIcon, XIcon, CheckIcon, PlusIcon, Filter as FilterIcon } from "lucide-react"
...
const getOperatorsForField = <T = unknown,>(
  ...
  return (fieldType in operators ? operators[fieldType] : undefined) ?? operators.select ?? []
}
  ...
  const renderOptionItem = (option: FilterOption<T>, overallIndex: number) => {
    ...
    return (
      <DropdownMenuItem
        ...
        className={cn(
          "data-highlighted:bg-accent data-highlighted:text-accent-foreground cursor-pointer select-none",
          option.className
        )}
        onSelect={(e) => {
      ...
      >
        <div
          className={cn(
            "mr-2 flex h-4 w-4 shrink-0 items-center justify-center rounded-xs border border-primary transition-colors",
            isSelected
              ? "bg-primary text-primary-foreground"
              : "border-muted-foreground/40 opacity-50"
          )}
        >
          {isSelected && <CheckIcon className="h-3 w-3 text-current stroke-[2.5]" />}
        </div>
...
export function Filters<T = unknown>({
  ...
  const totalActiveCount = useMemo(() => {
    return filters.reduce((acc, f) => {
      if (Array.isArray(f.values)) return acc + f.values.length
      return acc + (f.values !== undefined && f.values !== null && f.values !== "" ? 1 : 0)
    }, 0)
  }, [filters])

  const triggerButton = (trigger as React.ReactElement) ?? (
    <Button
      variant="outline"
    ...
    >
      ...
      <span>{mergedI18n.addFilter || "Filter"}</span>
      {totalActiveCount > 0 && (
        <Badge
          variant="secondary"
  ...
  const contextValue = useMemo<FilterContextValue>(
    ...
    [variant, size, radius, mergedI18n, className, showSearchInput, trigger, allowMultiple]
  )
      ...
      >
        {selectableFields.length > 0 && (
          ...
          >
            ...
            <DropdownMenuContent
              className={cn("w-[240px]", menuPopupClassName)}
              align="start"
            >
              {/* Active filters inside dropdown (Odoo style) */}
              {filters.length > 0 && (
                <div className="p-2 border-b border-border space-y-1.5">
                  ...
                  <div className="flex flex-wrap gap-1 max-h-28 overflow-y-auto">
                    {filters.map((filter) => {
                      const field = fieldsMap[filter.field]
                      if (!field) return null
                      const valuesList = Array.isArray(filter.values)
                        ? filter.values
                        : [filter.values]
                      const labelStr = valuesList
                        .map((v) => {
                      ...
                      return (
                        <span
                          key={filter.id}
                          className="inline-flex items-center gap-1 px-1.5 py-0.5 rounded-sm bg-secondary text-secondary-foreground text-xs border border-border/60 max-w-full"
                        >
                          <span className="font-medium text-foreground/90 shrink-0">
                            {field.label}:
                          </span>
                          <span className="truncate max-w-[120px]">
                            {labelStr || filter.operator}
                          </span>
                          <button
                            type="button"
                            onClick={(e) => {
                            ...
                            className="hover:text-destructive cursor-pointer ml-0.5 shrink-0"
                          >
                          ...
                          return (
                            ...
                            >
                              <DropdownMenuSubTrigger
                                ...
                                className="data-popup-open:bg-accent data-popup-open:text-accent-foreground data-highlighted:bg-accent data-highlighted:text-accent-foreground flex items-center justify-between"
                              >
                                ...
                                {activeCount > 0 && (
                                  <Badge
                                    variant="secondary"
                                    className="ml-auto rounded-sm px-1 py-0 text-[10px] font-normal"
                                  >
                                    {activeCount}
                                  </Badge>
                                )}
        ...
        {!hideChipsOutside &&
          filters.map((filter) => {
            ...
            if (!field) return null
            return (
              <ButtonGroup key={filter.id} className="">
