...
const ShippingAddressForm: React.FC<ShippingAddressFormProps> = (props) => {
    ...
    const renderFields = (values: any, fieldMeta: any, errors: any) => {
        ...
        const getError = (name: string) => form
            ? getFieldError({ state: { meta: fieldMeta?.[name] } })
            : errors?.[name]?.message || errors?.[name];

        const setFieldValue = (field: string, value: any) => {
            if (form) form.setFieldValue(field, value);
            else props.setValue?.(field, value);
        };
        ...
        const renderTextField = (name: string, type = 'text', placeholder?: string, dir?: 'ltr') => (
            form ? (
        ...
        const renderPhoneField = () => form ? (
            <form.Field name="recipientPhone">
                {(field: any) => (
                    <Input
                        ...
                        value={field.state.value || ''}
                        onChange={(event) => {
                            if (/^\d*$/.test(event.target.value)) field.handleChange(event.target.value);
                        }}
                        onBlur={field.handleBlur}
            ...
            <Input
                ...
                onChange={(event) => {
                    if (/^\d*$/.test(event.target.value)) props.handleChange?.(event);
                }}
                placeholder="01xxxxxxxxx"
                dir="ltr"
            />
            ...
            <div className="space-y-4">
                ...
                <div className={`p-4 rounded-lg space-y-4 animate-fadeIn border ${isGift ? 'bg-purple-50 border-purple-200' : 'bg-gray-50 border-gray-200'}`}>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <FormField label={isGift ? 'اسم المستلم*' : 'الاسم بالكامل*'} htmlFor="recipientName" error={getError('recipientName')}>
                            {renderTextField('recipientName', 'text', 'الاسم ثلاثي')}
                        </FormField>
                        <FormField label={isGift ? 'هاتف المستلم*' : 'رقم الهاتف*'} htmlFor="recipientPhone" error={getError('recipientPhone')}>
                            {renderPhoneField()}
                        </FormField>
                    </div>

                    <FormField label={isGift ? 'عنوان المستلم بالتفصيل*' : 'العنوان بالتفصيل*'} htmlFor="recipientAddress" error={getError('recipientAddress')}>
                        {renderTextField('recipientAddress', 'text', 'الشارع، رقم المبنى، الشقة، علامة مميزة')}
                    </FormField>
                        ...
                        <FormField label={isGift ? 'محافظة المستلم*' : 'المحافظة (لحساب الشحن)*'} htmlFor="governorate" error={getError('governorate')}>
                            {form ? (
                                <form.Field name="governorate">
                                    {(field: any) => (
                                        <Select id="governorate" value={field.state.value || ''} onChange={(event) => field.handleChange(event.target.value)} onBlur={field.handleBlur}>
                                ...
                                <Select id="governorate" name="governorate" value={values?.governorate || ''} onChange={props.handleChange}>
                                    <option value="" disabled>-- اختر المحافظة --</option>
                                    {EGYPTIAN_GOVERNORATES.map(gov => <option key={gov} value={gov}>{gov}</option>)}
                                </Select>
                            )}
                        </FormField>
                        <FormField label={isGift ? 'البريد الإلكتروني للمستلم' : 'البريد الإلكتروني (للمتابعة)'} htmlFor="recipientEmail" error={getError('recipientEmail')}>
                            {renderTextField('recipientEmail', 'email', 'example@mail.com', 'ltr')}
                        </FormField>
                    </div>
                            ...
                            <FormField label="رسالة الهدية (سنطبعها في كارت أنيق)" htmlFor="giftMessage">
                                {form ? (
                                    <form.Field name="giftMessage">
                                        {(field: any) => <Textarea id="giftMessage" rows={3} value={field.state.value || ''} onChange={(event) => field.handleChange(event.target.value)} onBlur={field.handleBlur} placeholder="اكتب رسالتك الرقيقة هنا لتصل مع الهدية..." />}
                                    </form.Field>
                                ) : (
                                    <Textarea id="giftMessage" name="giftMessage" rows={3} value={values?.giftMessage || ''} onChange={props.handleChange} placeholder="اكتب رسالتك الرقيقة هنا لتصل مع الهدية..." />
                                )}
                            </FormField>
                            <div>
                                <label className="flex items-center gap-2 text-sm cursor-pointer select-none">
                                    {form ? (
                                        <form.Field name="sendDigitalCard">
                                            {(field: any) => <input id="sendDigitalCard" type="checkbox" checked={Boolean(field.state.value)} onChange={(event) => field.handleChange(event.target.checked)} onBlur={field.handleBlur} className="h-4 w-4 rounded border-gray-300 text-primary focus:ring-primary" />}
                                        </form.Field>
                                    ) : (
                                        <input id="sendDigitalCard" type="checkbox" name="sendDigitalCard" checked={Boolean(values?.sendDigitalCard)} onChange={props.handleChange} className="h-4 w-4 rounded border-gray-300 text-primary focus:ring-primary" />
                                    )}
    ...
    if (form) {
        return <form.Subscribe selector={(state: any) => ({ values: state.values, fieldMeta: state.fieldMeta })}>{(state: any) => renderFields(state.values, state.fieldMeta, {})}</form.Subscribe>;
    }
