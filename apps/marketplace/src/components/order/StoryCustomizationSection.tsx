                            ...
                            return (
                                <>
                                    <FormField label="الهدف من القصة*" htmlFor="storyValue" error={getFieldError(field)}>
                                        <Select id="storyValue" value={storyValue || ''} onChange={(event) => field.handleChange(event.target.value)} onBlur={field.handleBlur}>
                                            <option value="" disabled>-- اختر هدفًا --</option>
                                            {showPredefinedGoals && storyGoals.map(goal => <option key={goal.key} value={goal.key}>{goal.title}</option>)}
                                            {showCustomGoal && <option value="custom">هدف آخر (مخصص)</option>}
                                        </Select>
                                    </FormField>
                                    {showCustomGoal && storyValue === 'custom' && (
                                        <form.Field name="customGoal">
                                            {(customGoalField: any) => <FormField label="الهدف المخصص*" htmlFor="customGoal" error={getFieldError(customGoalField)} className="mt-4"><Textarea id="customGoal" value={customGoalField.state.value || ''} onChange={(event) => customGoalField.handleChange(event.target.value)} onBlur={customGoalField.handleBlur} rows={3} placeholder="اكتب الهدف الذي تريد التركيز عليه في القصة..." /></FormField>}
                                        </form.Field>
                                    )}
                                </>
                            );
