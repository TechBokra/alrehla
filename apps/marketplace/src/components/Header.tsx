...
import { Bell, LayoutDashboard, Menu, ShoppingCart, X } from 'lucide-react';
...
const Header: React.FC = () => {
  ...
  useEffect(() => {
    const handleClickOutside = (event: MouseEvent) => {
      const clickedInside = Object.values(menusRef.current).some((ref) =>
        (ref as HTMLElement | null)?.contains(event.target as Node),
      );
      if (!clickedInside) {
        setOpenMenu(null);
      }
  ...
  return (
    <header
      className={`bg-background/80 backdrop-blur-md border-b sticky top-0 z-40 ${headerStyle}`}
      dir="rtl"
    >
          ...
          <div className="flex items-center gap-1 sm:gap-2">
            ...
            {isLoggedIn ? (
              <>
                <div
                  ref={(el) => {
                    menusRef.current['notifications'] = el;
                  }}
                  className="relative"
                >
                  <Button
              ...
              <Button as={Link} to="/login" size="sm" className="hidden lg:inline-flex">
                تسجيل الدخول
              </Button>
            )}
