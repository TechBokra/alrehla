...
import {
  ...
  useAdminMaintenanceSettings,
  useAdminLibraryPricingSettings,
} from '../../hooks/queries/admin/useAdminSettingsQuery';
...
const AdminSettingsPage: React.FC = () => {
  ...
  const { data: libraryPricingData, isLoading: libPricingLoading } =
    useAdminLibraryPricingSettings();
  ...
  const [libraryPricing, setLibraryPricing] = useState<LibraryPricingSettings>({
    id: 1,
  ...
  const handleBrandingChange = (fieldKey: string, value: any) => {
    const oldValue =
      branding[fieldKey as keyof SiteBranding] ||
      (initialBranding && (initialBranding as any)[fieldKey]);
  ...
  const handleSeedDatabase = async () => {
    if (
      !window.confirm(
        'تحذير: هذا الإجراء سيقوم بإعادة تعيين كافة نصوص الموقع وإعداداته إلى القيم الافتراضية (البيانات الوهمية). هل أنت متأكد؟',
      )
    )
      return;
  ...
  const handleMergeDuplicates = async () => {
    if (
      !window.confirm(
        'سيقوم هذا الإجراء بفحص جميع ملفات الأطفال، ودمج الملفات التي تحمل نفس الاسم وتتبع لنفس الأب في ملف واحد، مع نقل كافة الطلبات والحجوزات إليه. هل أنت متأكد من المتابعة؟',
      )
    )
      return;
    ...
    try {
      ...
      addToast(
        `تمت العملية بنجاح! تم دمج ${result.mergedCount} مجموعة، وحذف ${result.deletedCount} ملف مكرر.`,
        'success',
      );
    } catch (error: any) {
      ...
      <Tabs defaultValue="branding">
        <TabsList className="mb-8 flex-wrap h-auto">
          <TabsTrigger value="branding">
            <ImageIcon className="ml-2 w-4 h-4" /> هوية الموقع
          </TabsTrigger>
          <TabsTrigger value="communication">
            <Mail className="ml-2 w-4 h-4" /> التواصل
          </TabsTrigger>
          <TabsTrigger value="pricing">
            <DollarSign className="ml-2 w-4 h-4" /> ماليات التدريب
          </TabsTrigger>
          <TabsTrigger value="library_pricing">
            <Library className="ml-2 w-4 h-4" /> ماليات المكتبة
          </TabsTrigger>
          <TabsTrigger value="permissions">
            <Shield className="ml-2 w-4 h-4" /> الصلاحيات
          </TabsTrigger>
          <TabsTrigger value="maintenance">
            <PenTool className="ml-2 w-4 h-4" /> تنبيهات الموقع
          </TabsTrigger>
          <TabsTrigger value="data">
            <Database className="ml-2 w-4 h-4" /> البيانات
          </TabsTrigger>
        </TabsList>
        ...
        <TabsContent value="branding" className="space-y-6">
          <Card>
            <CardHeader>
              <CardTitle>الشعار والصورة الرئيسية</CardTitle>
            </CardHeader>
          ...
          <Card>
            <CardHeader>
              <CardTitle>صور بطاقات الأقسام (الصفحة الرئيسية)</CardTitle>
            </CardHeader>
          ...
          <Card>
            <CardHeader>
              <CardTitle>صور الصفحات الداخلية</CardTitle>
            </CardHeader>
            <CardContent className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <ImageUploadField
                label="هيرو صفحة من نحن"
        ...
        <TabsContent value="communication" className="space-y-6">
          <Card>
            ...
            <CardContent className="space-y-4">
              <FormField label="فيسبوك" htmlFor="fb">
                <Input
                  id="fb"
              ...
              <FormField label="تويتر" htmlFor="tw">
                <Input
                  id="tw"
                  value={socials.twitter_url}
                  onChange={(e) => setSocials({ ...socials, twitter_url: e.target.value })}
                />
              </FormField>
              <FormField label="انستجرام" htmlFor="ig">
                <Input
                  id="ig"
          ...
          <div className="flex justify-end">
            <Button onClick={handleCommunicationSubmit} icon={<Save />}>
              حفظ التواصل
            </Button>
          </div>
        </TabsContent>
        ...
        <TabsContent value="pricing" className="space-y-6">
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                هذه الإعدادات تحدد كيف يتم حساب السعر النهائي لخدمات وباقات المدربين.
              </CardDescription>
            </CardHeader>
            <CardContent className="space-y-6">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <FormField label="نسبة المنصة (مُعامل الضرب)" htmlFor="company_percentage">
                  <Input
                    ...
                    type="number"
                    step="0.1"
                    value={pricing.company_percentage}
                    onChange={(e) =>
                      setPricing({ ...pricing, company_percentage: parseFloat(e.target.value) })
                    }
                  />
                  <p className="text-xs text-muted-foreground mt-1">
                    مثال: 1.2 تعني زيادة 20% على سعر المدرب.
                  </p>
                </FormField>
                <FormField label="الرسوم الإدارية الثابتة (ج.م)" htmlFor="fixed_fee">
                  <Input
                    id="fixed_fee"
                    type="number"
                    value={pricing.fixed_fee}
                    onChange={(e) =>
                      setPricing({ ...pricing, fixed_fee: parseFloat(e.target.value) })
                    }
                  />
              ...
              <div className="p-4 bg-blue-50 rounded-lg border border-dashed border-blue-200">
                ...
                <div className="text-lg font-mono text-center bg-white p-3 rounded border">
                  السعر النهائي = (سعر المدرب * {pricing.company_percentage}) + {pricing.fixed_fee}{' '}
                  ج.م
                </div>
              </div>

              <div className="flex justify-end">
                <Button onClick={handlePricingSubmit} icon={<Save />}>
                  حفظ إعدادات التدريب
                </Button>
              </div>
            </CardContent>
        ...
        <TabsContent value="library_pricing" className="space-y-6">
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                هذه الإعدادات تحدد كيف يتم حساب السعر النهائي للكتب والمنتجات المضافة بواسطة دور
                النشر.
              </CardDescription>
            </CardHeader>
            <CardContent className="space-y-6">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <FormField label="نسبة المنصة (مُعامل الضرب)" htmlFor="lib_company_percentage">
                  <Input
                    id="lib_company_percentage"
                    type="number"
                    step="0.1"
                    value={libraryPricing.company_percentage}
                    onChange={(e) =>
                      setLibraryPricing({
                        ...libraryPricing,
                  ...
                  />
                  <p className="text-xs text-muted-foreground mt-1">
                    مثال: 1.3 تعني زيادة 30% على صافي سعر دار النشر.
                  </p>
                </FormField>
                <FormField label="الرسوم الثابتة لكل كتاب (ج.م)" htmlFor="lib_fixed_fee">
                  <Input
                    id="lib_fixed_fee"
                    type="number"
                    value={libraryPricing.fixed_fee}
                    onChange={(e) =>
                      setLibraryPricing({
                        ...libraryPricing,
                        fixed_fee: parseFloat(e.target.value),
                      })
                    }
                  />
                  <p className="text-xs text-muted-foreground mt-1">
                    تغطي تكاليف المعالجة والتشغيل.
                  </p>
                </FormField>
              </div>
              ...
              <div className="p-4 bg-purple-50 rounded-lg border border-dashed border-purple-200">
                ...
                <div className="text-lg font-mono text-center bg-white p-3 rounded border">
                  سعر العميل = (صافي الناشر * {libraryPricing.company_percentage}) +{' '}
                  {libraryPricing.fixed_fee} ج.م
                </div>
              </div>

              <div className="flex justify-end">
                <Button onClick={handleLibraryPricingSubmit} icon={<Save />}>
                  حفظ إعدادات المكتبة
                </Button>
              </div>
            </CardContent>
        ...
        <TabsContent value="maintenance" className="space-y-6">
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                عرض شريط ثابت في أعلى الموقع لتنبيه الزوار بوجود صيانة أو تطوير.
              </CardDescription>
            </CardHeader>
            <CardContent className="space-y-6">
              <div className="flex items-center gap-2">
                <Checkbox
                  ...
                  onCheckedChange={(checked) =>
                    setMaintenance((prev) => ({ ...prev, isActive: !!checked }))
                  }
                />
                <label htmlFor="maintenance-active" className="text-sm font-medium">
                  تفعيل الشريط
                </label>
              </div>
              ...
              <div className="flex justify-end">
                <Button
                  ...
                  loading={updateMaintenanceSettings.isPending}
                  icon={<Save />}
                >
                  حفظ الإعدادات
                </Button>
        ...
        <TabsContent value="data" className="space-y-6">
          <Card className="border-t-4 border-yellow-400">
            <CardHeader>
              ...
              <CardDescription>أدوات إصلاح وتنظيف قاعدة البيانات.</CardDescription>
            </CardHeader>
            <CardContent className="space-y-6">
              {/* أداة دمج التكرارات */}
              <div className="bg-blue-50 p-4 rounded-lg border border-blue-200 flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-blue-700 mt-1">
                      يصلح مشكلة تكرار ملف الطفل عند الطلب المتكرر. يقوم بدمج الملفات المتشابهة ونقل
                      سجلاتها لملف واحد.
                    </p>
                  </div>
                </div>
                <Button
                  ...
                  loading={isMerging}
                  variant="outline"
                  className="border-blue-300 text-blue-800 hover:bg-blue-100 whitespace-nowrap"
                >
                  فحص ودمج التكرارات
                </Button>
              </div>
              ...
              <div className="bg-yellow-50 p-4 rounded-lg border border-yellow-200 flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-yellow-700 mt-1">
                      إعادة تعيين المحتوى والإعدادات للقيم الافتراضية. (تحذير: يحذف التعديلات
                      الحالية)
                    </p>
                  </div>
                </div>
                <Button
                  ...
                  loading={isSeeding}
                  variant="outline"
                  className="border-yellow-300 text-yellow-800 hover:bg-yellow-100 whitespace-nowrap"
                  icon={<RefreshCw />}
                >
