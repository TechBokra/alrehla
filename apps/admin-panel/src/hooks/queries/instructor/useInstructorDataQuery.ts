...
export const useInstructorData = () => {
    ...
    return useQuery({
        ...
        queryFn: async () => {
            ...
            const bookingResult = await listBookings(apiClient, { instructorId: currentInstructor.id, pageSize: 100 });
            ...
            const [
                ...
                bookingIds.length > 0
                    ? supabase.from('session_attachments').select('*').in('booking_id', bookingIds).then(res => res.data as SessionAttachment[] || [])
                    : Promise.resolve([])
            ]);
